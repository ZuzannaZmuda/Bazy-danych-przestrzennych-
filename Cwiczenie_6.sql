...
ALTER SCHEMA schema_name RENAME TO Zmuda;

--Przykład 1 - ST_Intersects Przecięcie rastra z wektorem.
CREATE TABLE Zmuda.intersects AS
SELECT a.rast, b.municipality
FROM rasters.dem AS a, vectors.porto_parishes AS b
WHERE ST_Intersects(a.rast, b.geom) AND b.municipality ilike 'porto';

--Dodanie serial primary key:
alter table Zmuda.intersects add column rid SERIAL PRIMARY KEY;
...
SELECT AddRasterConstraints('Zmuda'::name,'intersects'::name,'rast'::name);

--Przykład 2 - ST_Clip
--Obcinanie rastra na podstawie wektora.
CREATE TABLE Zmuda.clip AS
...
CREATE TABLE Zmuda.porto_parishes AS WITH r AS (
...
DROP TABLE Zmuda.porto_parishes; --> drop table porto_parishes first CREATE TABLE schema_name.porto_parishes AS
WITH r AS (
SELECT rast FROM rasters.dem
LIMIT 1 )
...
DROP TABLE Zmuda.porto_parishes; --> drop table porto_parishes first 
CREATE TABLE Zmuda.porto_parishes AS
WITH r AS (
SELECT rast FROM rasters.dem
LIMIT 1 )
...
create table Zmuda.intersection as
...
CREATE TABLE Zmuda.dumppolygons AS
SELECT
...
--Przykład 10 - ST_TPI
create table Zmuda.tpi30 as select ST_TPI(a.rast,1) as rast from rasters.dem a;
CREATE INDEX idx_tpi30_rast_gist ON Zmuda.tpi30 USING gist (ST_ConvexHull(rast));
--Dodanie constraintów:
SELECT AddRasterConstraints('Zmuda'::name,'tpi30'::name,'rast'::name);

--problem do samodzielnego rozwiazania 
CREATE TABLE Zmuda.tpi30_porto AS
SELECT ST_TPI(a.rast,1) AS rast
FROM rasters.dem AS a, vectors.porto_parishes AS b
WHERE ST_Intersects(a.rast, b.geom) AND b.municipality ILIKE 'porto';

CREATE INDEX idx_tpi30_porto_rast_gist ON Zmuda.tpi30_porto USING gist (ST_ConvexHull(rast));

SELECT AddRasterConstraints('Zmuda'::name,'tpi30_porto'::name,'rast'::name);

--Algebra map
CREATE TABLE Zmuda.porto_ndvi AS
...
CREATE INDEX idx_porto_ndvi_rast_gist ON Zmuda.porto_ndvi USING gist (ST_ConvexHull(rast));

SELECT AddRasterConstraints('Zmuda'::name,'porto_ndvi'::name,'rast'::name);

--Przykład 2 – Funkcja zwrotna
--W pierwszym kroku należy utworzyć funkcję, które będzie wywołana później:
create or replace function Zmuda.ndvi(
...
CREATE TABLE Zmuda.porto_ndvi2 AS
WITH r AS (SELECT a.rid,ST_Clip(a.rast, b.geom,true) AS rast FROM rasters.landsat8 AS a, vectors.porto_parishes AS b
WHERE b.municipality ilike 'porto' AND ST_Intersects(b.geom,a.rast))
SELECT r.rid,ST_MapAlgebra(r.rast, ARRAY[1,4],'Zmuda.ndvi(double precision[],integer[],text[])'::regprocedure,
'32BF'::text) AS rast FROM r;

CREATE INDEX idx_porto_ndvi2_rast_gist ON Zmuda.porto_ndvi2 USING gist (ST_ConvexHull(rast));

SELECT AddRasterConstraints('Zmuda'::name,'porto_ndvi2'::name,'rast'::name);

--Eksport danych
--Przykład 1 - ST_AsTiff
SELECT ST_AsTiff(ST_Union(rast)) FROM Zmuda.porto_ndvi;

--Przykład 2 - ST_AsGDALRaster
SELECT ST_AsGDALRaster(ST_Union(rast), 'GTiff', ARRAY['COMPRESS=DEFLATE', 'PREDICTOR=2', 'PZLEVEL=9'])
FROM Zmuda.porto_ndvi;
